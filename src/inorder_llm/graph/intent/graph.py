...
from .nodes import BuildPlanNode, ClarificationNode, FinalizeNode, MainIntentNode, SubIntentNode, ValidatePlanNode
...
class IntentGraph(BaseGraph[IntentGraphState]):
    ...
    def build(self):
        ...
        builder.add_conditional_edges("main_intent", route_main_intent, {"order": "sub_intent", "qa": "build_plan", "ambiguous": "clarification"})
        builder.add_edge("sub_intent", "build_plan")
        builder.add_edge("build_plan", "validate_plan")
        builder.add_edge("validate_plan", "finalize")
        builder.add_edge("clarification", "build_plan")
