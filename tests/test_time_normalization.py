...
def test_fixed_time_is_classified_and_timezone_added():
    result = normalize_time_entity(
        _time({"start": "2026-08-16 15:00", "end": "2026-08-16 15:00"}, "明天下午三点")
    )
...
def test_closed_range_and_open_range_are_supported():
    closed = normalize_time_entity(_time({"start": "2026-08-16 06:00", "end": "2026-08-16 12:00"}))
    open_ended = normalize_time_entity(_time({"start": "", "end": "2026-08-16 12:00"}))
...
@pytest.mark.parametrize(
    ...
    [
        {"start": "", "end": ""},
        {"start": "not-time", "end": "2026-08-16 12:00"},
        {"start": "2026-08-16 13:00", "end": "2026-08-16 12:00"},
    ],
)
...
def test_time_normalization_is_pure():
    entity = _time({"start": "2026-08-16 12:00", "end": "2026-08-16 12:00"})
    original = dict(entity.attributes)
    normalize_time_entity(entity)
    assert dict(entity.attributes) == original


def test_reducer_accepts_time_without_context():
