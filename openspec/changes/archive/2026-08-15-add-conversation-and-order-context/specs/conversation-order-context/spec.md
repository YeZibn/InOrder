...
系统 SHALL 提供 `HistoryConversation` 保存有序的 user、assistant 和 system 对话回合，并支持获取最近回合和转换为 LLM 消息格式。
...
系统 SHALL 提供单一 active `OrderContext`，覆盖 pickup/dropoff、联系人、货物、车型、时间、支付、发票、服务和备注等订单字段，并支持稳定序列化。

#### Scenario: Empty context
- **WHEN** 创建新的 OrderContext
- **THEN** 所有可选单值字段为空、集合字段为空，且可序列化为 JSON-compatible 字典

#### Scenario: Session combines history and order
- **WHEN** 创建 ConversationSession
- **THEN** 会话同时持有 HistoryConversation 和 active OrderContext，二者可独立更新

### Requirement: Entity action reduction

系统 SHALL 将 Entity 的 `set`、`add`、`remove` 和 `replace` action 应用到 OrderContext，并返回更新后的上下文。
...
上下文和 reducer SHALL 只进行内存状态转换，不查询历史订单、不写入数据库、不创建或确认订单。

#### Scenario: Reduction is pure in-memory
- **WHEN** 对 Entity 列表执行 reducer
